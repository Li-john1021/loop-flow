...
| `spec` | 晋升后 | 从已批准完整 Plan 编译并冻结 Cycle Spec | Plan 未批准、批注未关闭或指纹/版本过期时拒绝；幂等返回 |
| `run [WU-id\|--step\|all]` | 核心 | 执行已冻结合同，默认执行 `all`；Tier 1 执行已批准任务卡 | 完整模式需冻结 Spec，Tier 1 需批准任务卡；单步不能跳过 Review、测试或安全门 |
...
4. 完整模式的状态转移是 `approve -> spec -> run`：`approve` 签署意图，`spec` 冻结可执行合同，`run` 只执行已冻结合同。Tier 1 通过批准任务卡后可直接 `run`。
5. `spec` 必须运行完整交叉校验；已有 Spec 指纹一致时不重复编译，Plan 版本或指纹变化时保留旧 Spec 并生成新的冻结版本。`run` 遇到陈旧 Spec 才拒绝执行并要求重新运行 `spec`。
6. `run --step` 只用于调试或恢复一个已授权步骤，不能改变既定 Review 顺序、跳过测试或扩大路径。
7. `validate` 失败返回 `revise`、`planning_review` 或 `blocked`，不能包装成 `done`。

## 不对用户暴露的内部动作

`dispatch`、`code-review`、`spec-review`、`build`、`test`、`audit` 和 `trace` 是 `run` 的内部阶段；`compile-spec` 由用户可见的 `spec` 命令承载。宿主可为排障提供别名，但别名必须转发到本契约，不能另建状态机。

## 宿主映射

- Claude Code：可用 `/loop-flow plan`、`/loop-flow spec`、`/loop-flow run` 等 Skill 参数；也可继续自然语言触发。
- Codex：可用 `$loop-flow` 加命令参数或自然语言。
- Gemini CLI：使用其 Skill 发现和命令入口映射。
- 其他宿主：至少保留 `plan`、`annotate`、`approve`、`spec`、`run`、`status`、`resume` 的自然语言降级。
